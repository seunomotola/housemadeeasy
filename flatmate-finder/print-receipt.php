 ...
 include 'inc/session.php';
    ...
    $house_img1=$_SESSION['house_img1'];
           ...
           // $sql="UPDATE p set amount_paid='$amount2', status = 'yes', reference='$referenced2',  date_paid=NOW() where portalid='$portalid' AND term='$term'";
            ...
            $sql = "INSERT into flatmate_finder_payment_history(user_email, pno, fname, lname, agent, agent_img, agent_pno, agent_email, location, house_location, type, house_name, house_price, house_img1, house_img2, house_img3, house_img4, house_desc, amenities, house_label, distance, kitchen, bathroom, door, fence, water_source, amount_paid, status, date_paid, reference, first_year_rent, second_year_rent, payment_expire, house_id, house_owner) values('$email2', '$pno', '$fname', '$lname', '$house_agent_session', '$house_agent_img_session', '$agent_pno', '$agent_email', '$house_location_session', '$house_exact_session', '$house_type', '$house_name_session', '$house_price_session', '$house_img1', '$house_img2_session', '$house_img3_session', '$house_img4_session', '$house_desc_session', '$amenities_session', '$house_label_session', '$distance_session', '$kitchen_session', '$bathroom_session', '$door_session', '$fence_session', '$water_source_session', '$amount2','yes', '$date_time', '$referenced', '$first_year_rent' ,'$second_year_rent', '', '$house_id', '$house_owner')";

            if(mysqli_query($con, $sql)){ ?> 
             <script>
             alert('Payment Successful ...');
             window.location.href='book.php?house_id=<?php echo $house_id; ?>';
            // window.location.href='https://www.housemadeeasy.org/book.php?house_id=<?php echo $house_id; ?>';
    </script>; 
<?php
