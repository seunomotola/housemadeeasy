 ...
 include 'inc/session.php';
      ...
      if (isset($_GET['reference'])) {
        ...
        $referenced=$_SESSION['reference'];
         //$total_price = isset($_SESSION['total_price']);
    ...
    $house_img1=$_SESSION['house_img1'];
      ...
      isset($_SESSION['total_price']) ? $_SESSION['total_price'] : '';
           ...
           // $sql="UPDATE p set amount_paid='$amount2', status = 'yes', reference='$referenced2',  date_paid=NOW() where portalid='$portalid' AND term='$term'";
            ...
            $sql = "INSERT into payment_history_urgent(user_email, pno, fname, lname, agent, agent_img, agent_pno, agent_email, location, house_location, type, house_name, house_img1, house_img2, house_img3, house_img4, house_desc, amenities, house_label, distance, kitchen, bathroom, door, fence, water_source, amount_paid, status, date_paid, reference, first_year_rent, second_year_rent, house_id, house_owner, user_id, negotiable, urgent, agentaffilate_id) values('$email2', '$pno', '$fname', '$lname', '$house_agent_session', '$house_agent_img_session', '$agent_pno', '$agent_email', '$house_location_session', '$house_exact_session', '$house_type', '$house_name_session', '$house_img1', '$house_img2_session', '$house_img3_session', '$house_img4_session', '$house_desc_session', '$amenities_session', '$house_label_session', '$distance_session', '$kitchen_session', '$bathroom_session', '$door_session', '$fence_session', '$water_source_session', '$amount2','yes', '$date_time', '$referenced', '$first_year_rent' ,'$second_year_rent', '$house_id', '$house_owner', '$user_id', '$negotiable', 'yes', '$agentaffilate_id_session')"; 

      if(mysqli_query($con, $sql)){ 
               if (isset($_COOKIE['referral_code'])) {?>
                     <script>
             alert('Payment Successfull');
             window.location.href='book-urgent-referral.php?house_id=<?php echo $house_id; ?>'; 
    </script>;
               <?php }else{?>
                    <script>
             alert('Payment Successfull');
             window.location.href='book-urgent.php?house_id=<?php echo $house_id; ?>'; 
    </script>;
               <?php }
